...
namespace StopWatch
{
    public partial class WorklogForm : Form
    {
        ...
        private void RemainingEstimateUpdated()
        {
            ...
            {
                rdEstimateAdjustLeave.Text = "Deixar inalterado";
            }
            else
            {
                rdEstimateAdjustLeave.Text = string.Format("Deixar como {0}", _RemainingEstimate);
            }

            if( TimeElapsed != null && RemainingEstimateSeconds > 0){
                rdEstimateAdjustAuto.Text = string.Format("Ajustar automaticamente (para {0})", calculatedAdjustedRemainingEstimate());
            }else {
                rdEstimateAdjustAuto.Text = "Ajustar automaticamente";
            }
        }
